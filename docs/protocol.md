...
manifest. C2PA is opt-in and only attaches to a real asset supplied with
`on-record c2pa --asset`; the ledger never fabricates a PNG or other media. The JSON bridge remains
intentionally easier to inspect and is not itself a C2PA file. Assertions include:
